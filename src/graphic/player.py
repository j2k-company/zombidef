...
class Player:
    def __init__(self, game_map: Map):
        ...
        self.command_buffer = Command([], [], None)
    ...
    def keyboard_control(self, button: int, offset_x, offset_y):
        ...
        match button:
            case Motion.build.value:
                self.command_buffer.build.append(Coordinate(global_position[0], global_position[1]))
            case Motion.attack.value:
                if self.attacking_block:
                    self.command_buffer.attack.append(
            ...
            case Motion.move_base.value:
                self.command_buffer.move_base = Coordinate(global_position[0], global_position[1])
